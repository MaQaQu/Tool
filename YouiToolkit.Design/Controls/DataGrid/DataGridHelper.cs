...
namespace YouiToolkit.Design
{
    public class DataGridHelper
    {
        ...
        private static void DataGrid_AutoGeneratingColumn(object sender, DataGridAutoGeneratingColumnEventArgs e)
        {
            ...
            var minWidth = e.Column.MinWidth;
            ...
            var canUserReorder = e.Column.CanUserReorder;
            var canUserResize = e.Column.CanUserResize;
            var canUserSort = e.Column.CanUserSort;

            bool isRenderTypeRegisted = false;

            var descriptor = e.PropertyDescriptor as PropertyDescriptor;
            foreach (Attribute attribute in descriptor.Attributes)
            {
                if (attribute is DataGridColumnIgnoreAttribute)
                {
                    visibility = Visibility.Collapsed;
                }
                if (attribute is DataGridColumnBindingAttribute bindingAttr)
                {
                    bindingMode = bindingAttr.BindingMode;
                    updateSourceTrigger = bindingAttr.UpdateSourceTrigger;
                }
                if (attribute is DisplayNameAttribute displayNameAttr)
                {
                    header = displayNameAttr.DisplayName;
                }
                if (attribute is DataGridColumnAttribute columnAttr)
                {
                    width = GridLengthUtil.ConvertToDataGridLength(columnAttr.Width);
                    minWidth = columnAttr.MinWidth;
                    readOnly = columnAttr.ReadOnly;
                    header = columnAttr.DisplayName;
                    canUserReorder = columnAttr.CanUserReorder;
                    canUserResize = columnAttr.CanUserResize;
                    canUserSort = columnAttr.CanUserSort;

                    if (columnAttr.RenderType == typeof(DataGridButtonGroupColumn))
                    {
                        var newColumn = new DataGridButtonGroupColumn()
                        {
                            Width = width,
                            //MinWidth = minWidth,
                            Header = header,
                        ...
                        e.Column = newColumn;
                        isRenderTypeRegisted = true;
                    }
                }
            }
            #endregion

            if (isRenderTypeRegisted)
            {
                return;
            }
