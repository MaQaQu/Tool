...
namespace YouiToolkit.Design
{
    internal class CheckIconConverter : IMultiValueConverter
    {
        ...
        {
            var width = values[0] as double? ?? 0d;
            var icon = values[1] as MessageBoxIcon? ?? MessageBoxIcon.None;
            var thickness = values[2] as double? ?? 0d;
            var path = "";
            ...
            switch (icon)
            {
                ...
                case MessageBoxIcon.Warning:
                    //path = $"M {0.5 * width},{0.1 * width} L 5,{0.9 * width} H {width - 5} Z M {0.5 * width},{0.3 * width} V {0.7 * width} M {0.5 * width},{0.8 * width} V {0.8 * width}";
                    path = $"M {0.5 * width},{0.2 * width} L 5,{0.8 * width} H {width - 5} Z M {0.5 * width},{0.4 * width} V {0.6 * width} M {0.5 * width},{0.7 * width} V {0.7 * width}";
                    break;
