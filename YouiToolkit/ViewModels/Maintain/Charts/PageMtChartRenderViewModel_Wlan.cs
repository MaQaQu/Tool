...
{
    public class PageMtChartRenderViewModel_Wlan
    {
        /// <summary>
        /// 画直线
        /// </summary>
        public PlotModel PlotWlan { get; set; }
        public PageMtChartRenderViewModel_Wlan()
        {
            PlotWlan = new PlotModel();
            //线条
            var lineSerial = new LineSeries() { Title = "直线实例" };
            lineSerial.Points.Add(new DataPoint(0, 0));
            lineSerial.Points.Add(new DataPoint(10, 10));
            PlotWlan.Series.Add(lineSerial);

            //函数sin(x)
            //var funcSerial = new FunctionSeries((x) => { return Math.Sin(x); }, 0, 10, 0.1, "y=sin(x)");
            //funcSerial.Title = "速度";
            //PlotAngle.Series.Add(funcSerial);
            ...
            PlotWlan.Axes.Add(leftAxis);
            PlotWlan.Axes.Add(bottomAxis);
            PlotWlan.Title = "Wlan";
        }
    }
}
